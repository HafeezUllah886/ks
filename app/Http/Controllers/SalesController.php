        ...
        try
        {
            ...
            foreach($ids as $key => $id)
            {
                if($request->amount[$key] > 0)
                {
                    ...
                    $exclusive = $request->qty[$key] * $request->price[$key];
            ...
            $discount = $request->discount;
            $taxValue = ($totalexclusive - $discount) * $request->tax / 100;

            $net = ($totalexclusive - $discount) + $taxValue ;

            $sale->update(
                [
                    'total'   => $net,
                    'exlusive'=> $totalexclusive - $discount,
                    'taxValue'=> $taxValue,
                ]
            );
